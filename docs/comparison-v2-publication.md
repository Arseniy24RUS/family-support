...
Страница не загружает полные тексты при первоначальном открытии. PDF получает адрес встроенного `iframe` только после явной команды пользователя. Для исходных DOCX по первому нажатию последовательно подключаются локальные сборки JSZip и docx-preview, затем запрашивается только выбранный файл и его содержимое формируется внутри страницы. При закрытии или переключении документа незавершённый запрос отменяется; остальные файлы корпуса не загружаются.

Рендеринг DOCX выполняется в браузере без отправки документа во внешний сервис. Альтернативные HTML-вставки DOCX отключены, а оригинал и PDF остаются доступны для скачивания. На узких экранах бумажная страница переводится в адаптивную ширину с переносом текста. JSON-манифест и лексические профили по-прежнему загружаются как обычные статические данные. Для сохранения приемлемого времени публикации не следует включать перерасчёт лексического профиля в каждый обычный запуск сайта.
