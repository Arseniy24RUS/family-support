...
test('прямая ссылка открывает документ без автоматической загрузки PDF', async ({ page }) => {
  ...
  await page.goto(`/documents.html?doc=${encodeURIComponent(id)}#document-library`);
  await expect(page.locator('#data-status')).not.toContainText('Загрузка', { timeout: 20_000 });
  await expect(page.locator('#strategy-viewer-content')).toBeVisible();
  await expect(page.locator('#strategy-pdf-frame')).toHaveAttribute('src', 'about:blank');
...
test('DOCX загружается и отображается внутри страницы только после нажатия', async ({ page }) => {
  ...
  await page.goto(`/documents.html?doc=${encodeURIComponent(strategyDocument.id)}#document-library`);
  await expect(page.locator('#data-status')).not.toContainText('Загрузка', { timeout: 20_000 });
...
test('страница документов фильтрует сразу по нескольким субъектам через чекбоксы', async ({ page }) => {
  const corpus = await page.request.get('/data/strategies.json').then((response) => response.json());
  const regions = corpus.documents.filter((item) => item.scope === 'regional').slice(0, 2).map((item) => item.territory);

  await page.goto('/documents.html');
  await expect(page.locator('#data-status')).not.toContainText('Загрузка', { timeout: 20_000 });
  await expect(page.locator('#documents-region-checklist input[name="document-region"]')).toHaveCount(89);
  for (const region of regions) await page.getByLabel(region, { exact: true }).check();

  await expect(page.locator('#documents-region-summary')).toContainText('Выбрано: 2 из 89');
  expect(await page.evaluate(() => new URL(location.href).searchParams.getAll('region'))).toEqual(regions);
  const visibleIds = await page.locator('#strategy-document-list [data-document-id]').evaluateAll((items) => items.map((item) => item.dataset.documentId));
  const byId = new Map(corpus.documents.map((item) => [item.id, item]));
  expect(visibleIds.length).toBeGreaterThanOrEqual(2);
  expect(visibleIds.every((id) => {
    const item = byId.get(id);
    return regions.includes(item.scope === 'municipal' ? item.parent_region : item.territory);
  })).toBe(true);

  await page.locator('#clear-document-regions').click();
  await expect(page.locator('#documents-region-summary')).toContainText('Все 89 субъектов');
  expect(await page.evaluate(() => new URL(location.href).searchParams.getAll('region'))).toEqual([]);
});
