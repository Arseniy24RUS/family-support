...
test('header объединяет логотипы организаций и открывает основные разделы платформы', async ({ page }) => {
  ...
  await expect(platformNav.locator('a')).toHaveCount(5);
  ...
  await expect(organizations.locator('a.institution-brand')).toHaveAttribute('href', 'https://isd-ras.ru/');
  ...
  const headerBox = await page.locator('.main-header__inner').boundingBox();
  const navBoxes = await platformNav.locator('a').evaluateAll((links) => links.map((link) => {
    const box = link.getBoundingClientRect();
    return { x: box.x, y: box.y, width: box.width, height: box.height };
  }));
  if (viewportWidth > 760) {
    expect(headerBox.height).toBeGreaterThanOrEqual(120);
    expect(productStyle.fontSize).toBeGreaterThanOrEqual(16);
    expect(Math.max(...navBoxes.map((box) => box.y)) - Math.min(...navBoxes.map((box) => box.y))).toBeLessThan(2);
    const activeStyle = await platformNav.locator('a[aria-current="page"]').evaluate((node) => ({
      backgroundColor: getComputedStyle(node).backgroundColor,
      underlineHeight: Number.parseFloat(getComputedStyle(node, '::after').height)
    }));
    expect(activeStyle.backgroundColor).not.toBe('rgba(0, 0, 0, 0)');
    expect(activeStyle.underlineHeight).toBeGreaterThanOrEqual(3);
  } else {
    expect(new Set(navBoxes.map((box) => Math.round(box.x))).size).toBe(2);
  }
